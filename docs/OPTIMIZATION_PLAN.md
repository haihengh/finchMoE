...
15. ~~**1.5** — the GDN projections were one int8 GEMV dispatch per token~~ — **done 2026-09-17, on by default**: a tiled int8 kernel took the 426-token prefill 29.17 s -> 20.78 s and a 2940-token prefill's projection work 82.6 s -> 22.3 s, gated by EvalPlus (0.951/0.921 against 0.945/0.909). Scope: 3.8's body only — 3.6's GDN8 tier carries the same per-token GEMV and needs the same call-site wiring
