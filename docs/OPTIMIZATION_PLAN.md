...
- **Measured 2026-09-11 (4.1's counters), Qwen 3.8, 54-token prompt, 64 tokens, `--temperature 0`:**
  ...
  **Recommendation, revised: do not build the prefetch or the widened pool.** This cycle proposed re-scoping item 2.1 around in-flight depth; the drive measurement refutes it, and it is recorded here rather than quietly dropped. Predicted experts are by construction either already resident — in which case prefetching them is free and pointless — or genuinely cold, in which case the prediction moves the read set toward the diverse regime this table prices at half the throughput of the repetitive one. The current operating point is a local optimum in which two caches absorb a deliberately repetitive read set, and every axis that looked like a lever is already at its knee.
