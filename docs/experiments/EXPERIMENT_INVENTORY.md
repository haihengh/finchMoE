...
| [KV-15](summaries/05-attention-and-kv-cache.md#kv-15) — Prefill reproducibility is a function of the chunk size, and the ranking's selections are not where it starts | New instrument `FQ_QSA_DUMP=<path>` fingerprints the ranking per full-attention layer (cell count, pooled blocks, FNV-1a of the selected indices), from `produceToken` after its wait and from the prefill chunk loop after a drain. Four runs of a 2940-token prompt at chunk **128 give four distinct outcomes** (all six pairs differ, 244,893-247,551 of 248,320 logits); four at **512 give one** (md5-identical). | The ranking dispatches either way — positions are >= 2051 both times — so the chunking, not the context length, is the variable, and the earlier past-2051 framing was a proxy for it. In a diverging chunk-128 pair, **0 of 276 prefill fingerprints differ** while 276 of 372 decode ones do, first at the first decode step; the logits dumped at the prefill/decode boundary differ in 246,623 elements, so the divergence is in the prefill output but not at its sampled selection points. | Open. Same shape as KV-14 (a tiling-dependent prefill race), so a shared-memory reuse hazard is the leading hypothesis. Confound to name: chunk 128 also takes 60% longer, so a {128, 256, 512} ladder is the discriminating experiment. |
