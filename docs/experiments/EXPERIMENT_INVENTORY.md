...
| [IO-21](summaries/01-model-install-and-expert-io.md#io-21) — Adding the GPU to the replay costs the drive 12%, and the per-read median is not a stable statistic | IO-20's one open cell, tested by **inversion**: the engine cannot be asked (no trace playback, so nop'ing the routed readback changes routing and the read population with it), so a dose-matched GPU ring reader (`gpu_load.swift`: 648.8 MiB per 147 ms at 16.7 GB/s — bytes and burst length, not a rate, because 3.8's 16.7 GB/s is compute-bound) was run **concurrently with the offline replay**, whose trace is then fixed by construction. Five order-alternated rounds on 3.8: **alone 146.6-148.2 ms/step against loaded 165.0-165.2**, +17.7 ms/step (**+12.0%**), rate **4.62 → 4.12 GB/s**, per-read thread time 1.97 → 2.27 ms. 3.6 under its own smaller dose separates 6/6 the same way: 43.7 → 47.1 (+8.0%), 5.46 → 5.03 GB/s. `gpu_load --no-dispatch` — the Metal ring allocated and page-touched but never read — shows the **full fault storm** (78-79k `minflt`) with **no slowdown**, so the faults belong to the allocation and the cost to the GPU reading. A five-rung ladder spanning **16x in delivered GPU bandwidth** (0.86 → 13.80 GB/s, five order-alternated rounds each) moves the penalty **+15.59 / +15.28 / +15.40 / +15.60 / +15.93 ms/step** — flat, so the cost is **per-activation, not per-byte**, and **~14% is a ceiling rather than a lower bound**: the engine's unmodelled `gpu_cb1` traffic buys nothing. Worth **~14%** of the engine's gap (0.30 of 2.16 ms per read). Unplanned: the replay's per-read distribution is **bimodal** (bulk at 0.13-0.52 ms, another at 2.1-8.4, median in the 4.1%-full valley), so p50 hops a full bucket on a few points of mixture while the mean moves 3% — `--hist-dump` added. | **IO-20's GPU-contention elimination is withdrawn**, and so is IO-21's own CPU null: `mem_load` was passed `gpu_load`'s `--gbps`, exited 2 on its first argument, and every "loaded" arm of it ran empty. Re-run with per-program flags it is **+7.2 / +9.5 ms** at the GPU's 3.40/13.57 GB/s doses — not null, though not a matched control either (four CPU cores against `gpu_load`'s 1.1%). ~86% of the gap remains; the specificity question is re-opened, the dose question is closed. |
| [IO-22](summaries/01-model-install-and-expert-io.md#io-22) — The drive's contention penalty is generic, not GPU-specific, and a third of it is thread pressure | IO-21's specificity question, matched on both axes it had left open: every arm interleaved in **one session with a rotating start** (its halves had run in different sessions whose alone arms differed by more than the effect, 184-194 against 167-173 ms/step), the CPU arm dose-matched by construction (`--burst-mb`/`--period-ms` shared; only the rate flag differs), two thread counts to separate threads from bytes, and a fourth arm burning four cores with **no memory traffic** (`yes`). Eight rounds of five arms on 3.8: alone 145.67 ms/step (spread only 143.7-148.7 within the session) against gpu **+18.43**, cpu4 **+13.78**, cpu1 +11.90, burn **+5.86**; in `thread_mean`, +0.31 / +0.24 / +0.19 / +0.14. | The penalty is **not GPU-specific** — CPU traffic costs 75% of the GPU's price at the same dose — thread count is not the driver (cpu4 minus cpu1 is 1.9 ms), and a third of the CPU arm's cost survives with no traffic at all, so it is scheduler pressure rather than bytes. That also explains IO-21's flat 16x ladder: the price follows the **presence** of activity, not its volume. Contention is worth up to a quarter to a third of the engine's 2.16 ms/read gap; whether the cells stack is untested. ~68% remains unexplained. |
...
| [KV-15](summaries/05-attention-and-kv-cache.md#kv-15) — Prefill reproducibility tracks how long the prefill *runs*; both earlier attributions refuted | New instrument `FQ_QSA_DUMP=<path>` fingerprints the ranking per full-attention layer (cell count, pooled blocks, FNV-1a of the selected indices), from `produceToken` after its wait and from the prefill chunk loop after a drain. The chunk ladder ({128,256,512} x 3 runs, one 2940-token prompt) showed a dose-response in divergence (3/2/1 distinct outcomes) that was *duration*: 343.9/269.9/215.0 s. The matched-duration control (chunk **512**, 4606 tokens, **341-351 s**) gave **3 distinct outcomes in 3 runs**, and the selector-less control (`FQ_QSA_OFF`, 354-360 s) gave a pair differing in **246,482 of 248,320 logits**. | Same 2940-token prompt: diverges at 344 s, not at 215 s, so length alone is out. Chunk 512: diverges at 348 s, not at 215 s, so chunk size alone is out. Chunk count disfavored (12 chunks @270 s < 9 chunks @348 s). A selector-less pair differing at 354 s kills the ranking-path isolation, whose null was taken at ~295 s. Elapsed prefill time survives every comparison (215 s never in 7 runs, 270 s in 2 of 3, 344-348 s in every pair tried). In a diverging chunk-128 pair, **0 of 276 prefill fingerprints differ** while 276 of 372 decode ones do, first at the first decode step. | Open — **both** the chunk-size reading and the ranking-path isolation are **withdrawn, refuted by their own controls**. **Resolved for the configuration that carried the evidence (2026-09-18).** Both earlier readings stay withdrawn. The row-fingerprint instrument (`FQ_ROW_HASH`) put the divergence inside a full-attention layer with the plane, the attention's q/k/v, the indexer's query and raw-key timeline **and** the pooled keys all bit-identical, leaving only `scores` between identical inputs and differing cells. Reading the ranking kernels found nothing; reading the *call site* found it — the prefill's indexer store applied both a `kRawOffset` binding and a `pos` while the kernel adds `pos·idxDim` itself, so every prefill key landed at `2·pos`: mis-pooling every block the prefill completes, and, past `maxContext/2` (2048 at 4k context — three positions from the `idxCapacity = 2051` boundary the hunt had been using), writing up to 456 KB past the timeline per full layer into memory the driver cannot associate with the dispatch. Fixed and pinned by `QSAIndexerTests/chunkFormPostsUseAbsolutePositions`; **four runs in two pairs at 358.9-361.6 s now agree bit-for-bit** where the old code never reproduced. **Still open:** a second, **intermittent** mechanism in the dense path — one divergent pair in five on the day (the `FQ_QSA_OFF` configuration, no indexer at all; the diverging pair was the fastest of the five, 264.5 s against 312-317 s) — first difference one row of layer 3's attention output with the q/k/v stage rows equal(247,480 of 248,320 logits) and agreed on the next (0 of 248,320). Its diverging map reads: first difference at layer 3 stage `attn` row 1024 with `qkv`/`krot`/`vrot` showing no differing row at that layer — the attention output differs on bit-identical q/k/v *stage* rows, the same landing the first indexer-on map produced. Because it is intermittent no deterministic geometry can be the mechanism; stages 12/13 (`kcache`/`vcache`) now hash the K/V cache — the copy's destination, never fingerprinted before — to separate the copy from the kernel. Its duration is also not a stable name for the machine's state: the same selector-off configuration measured 359.6 s on 2026-09-16 and 264.5 s on 2026-09-18. |
...
| [PF-18](summaries/06-prefill.md#pf-18) — What the prefill’s I/O actually costs, and the tile pipeline that does not change it | The routed-expert tile depth (`PrefillRoutedTileSchedulerConfig`, hardcoded `maxPendingDepth: 1`) became a runtime setting with its slot budget enforced. Five arms x 2 rounds over a 4x change in tiles in flight, on a 426-token prompt at chunk 512. | A 1.6% spread (28.84-29.30 s), identical bytes (33.11 GB), and **bit-identical logits on every arm**; back-to-back repeats flat too (28.98/28.98/29.01), so the page cache is not hiding anything. The runs did establish the prefill’s first real I/O breakdown (new `scope=prefill` counters line): reads occupy **10.59 s of 29.03 s (36%)** at **3.13 GB/s, the drive’s ceiling**, 6.6 in flight against decode’s 5.59. | The tile-depth hypothesis is **refuted**; the drive is idle 64% of the prefill as a *consequence* of reads that are already at full speed when they run. ~18.4 s of GPU/host work is unaccounted for, and the `gpu_*` counters read zero in the prefill scope — that split is the next measurement. | **Later in the same entry:** the GDN stack split four ways (`FQ_GDN_SPLIT=1`) put **95% of it in two projection stages** and 6% in the chunked scan; the projections turned out to be one int8 GEMV dispatch per token, and a tiled batch kernel took the 426-token prefill **29.17 s -> 20.78 s** and a 2940-token prefill's projection work **82.6 s -> 22.3 s** — on by default, gated by EvalPlus at 0.951/0.921 against 0.945/0.909. |
...
| [METH-16](summaries/09-validation-and-measurement-lessons.md#meth-16) — The bytes are an assumption until someone reads them twice | Nothing was changed; the install’s byte stability had simply never been tested. Three cold passes (`F_NOCACHE`, so the OS cache cannot serve pass 2) over 96.8 GiB of the 97 GiB install, hashing every 8 MiB block with BLAKE2b. | **37,266 block comparisons, zero differing blocks, zero short reads** — so the weights are not changing under the engine. The naive version of this test (no `F_NOCACHE` on a 7.9 GiB subset) passes while proving nothing, because the second read comes out of RAM. Paired with two further exclusions in KV-15: TSan finds no host-side race even on the exact diverging configuration, and both streamers verify short reads. | The read path is excluded from the long-prompt nondeterminism. TSan’s blind spot is named: the Metal driver is uninstrumented, so host-write/GPU-read hazards are invisible. |
