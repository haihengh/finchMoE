...
| Prefill row fingerprints (`FQ_ROW_HASH=<path>`) | unset, path | unset | Diagnostic, not a setting. Hashes each row of the residual plane on the GPU — one 64-bit FNV-1a per (layer, stage, row) at the layer's input, after its attention block, and after its routed tail — into a side buffer read back once at the prefill/decode boundary. It exists because a prefill chunk's rows share one command buffer, so a host read of an intermediate row would need a commit and a wait per chunk; on the runs this instrument is for, the wall clock is the correlate under investigation, so the hashing is done on the GPU and nothing is added to the run's shape. Cost is one small dispatch per (layer, stage) and a buffer of 8 bytes × 3 stages × layers per position (9 MiB at 8k context, 75 MiB at 64k). Diff two dumps with `tools/row-hash-diff.py a.bin b.bin`, which reports the first (layer, stage, row) that moved and what that stage implies. For the long-prompt nondeterminism it localized the first difference to layer 7's attention output at chunk 18 — chunks 0-17 are bit-identical ([KV-15](experiments/summaries/05-attention-and-kv-cache.md#kv-15)). |
