...
| Prefill row fingerprints (`FQ_ROW_HASH=<path>`) | unset, path | unset | Diagnostic, not a setting. Hashes each row on the GPU in twelve stages: the residual plane at three points of every layer (its input, after the attention block, after the routed tail), six inside the attention block of full-attention layers (the queries, the QSA selection, the attention core output, the block output after the gate and o_proj, and the keys and values after the RoPE and norm epilogue), and three inside the indexer (its query projection, the raw key timeline as this chunk writes it, and the **pooled keys** — rows there are blocks, not positions). They share one dimension, so one dump and one diff cover both layer kinds. One readback at the prefill/decode boundary, so nothing is added to the run’s shape; the buffer is 8 bytes x 12 stages x layers per position — 38 MB at an 8k prefill on the 125B. Diff two dumps with `tools/row-hash-diff.py a.bin b.bin`, which reports the first (layer, stage, row) that moved and what that stage implies. What it has established, in the order the stages were added: the first landings were a full-attention layer’s own output (layer 3 row 1024, later layer 7 at rows 2064 and 2304) with the layer’s input, its q/k/v projections and, at layer 3, its chosen cells all bit-identical; then, with the indexer’s stages added, the ranking’s own inputs — its query, its raw key timeline — were identical while the cells differed; and with the **pooled keys** hashed last, the first landing is now **identical pooled keys with differing cells**, which puts the locus in score → radix-select → cells-write and excludes the pool’s multi-chunk page assembly. Recorded as at least two meeting points, the ranking and the attention output. That locus was then traced one step further back — to the indexer *store* that fed the ranking, which was writing every prefill key at twice its position — and fixed; the pairs that diverged now reproduce bit-for-bit. See [KV-15](experiments/summaries/05-attention-and-kv-cache.md#kv-15). |
