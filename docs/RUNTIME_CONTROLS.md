...
| QSA selector off (`FQ_QSA_OFF=1`, or `RuntimeConfiguration(qsaIndexerEnabled: false)`) | unset, 1 | unset | Measurement tool, not a setting. Builds the runner with no sparse-block selector: every full-attention layer keeps every causally-visible cell on the dense path, at every position. It is the knob that first isolated the engine's long-context non-reproducibility to the ranking path — the same 2511-token prompt was bit-identical across two runs with it off and differed in 99.5% of logits with it on — **and that isolation has since been refuted by the knob itself**: at 4606 tokens (354-360 s of prefill) two selector-less runs differ in 246,482 of 248,320 logits. Treat it as a way to compare the two attention paths, not as a cure ([KV-15](experiments/summaries/05-attention-and-kv-cache.md#kv-15)). Prefer the configuration form: the environment variable is process-global, so it cannot be used from a parallel test. It is also the *only* way to reach a selector-less runner — an install built without indexer tensors cannot be loaded at all, because `validateQwen38Layers` requires them on every full layer. |
| Batched int8 projection (`FQ_INT8_GEMM=0` to disable) | 0, unset | **on** | A setting, not a diagnostic. The linear-attention (GDN) projections are int8 on every shipped Qwen 3.8 install, and the prefill used to feed them one GEMV per token — 426 re-reads of each weight matrix per chunk, ~46,000 dispatches, 82.6 s of projection work on a 2940-token prefill. On, they go through a tiled kernel that dequantizes the weight tile once and reuses it across the token dimension: **that workload drops to 22.3 s**, and end to end the 426-token prefill measures 29.17 s -> 20.78 s with decode unchanged. Verified before the default flipped: an oracle against the GEMV's own arithmetic read from the same bytes, and EvalPlus HumanEval at 0.951 base / 0.921 plus against 0.945 / 0.909 for the same install without it. `0` restores the per-token GEMV, which is also what any shape the tile cannot cover falls back to. See [PF-18](experiments/summaries/06-prefill.md#pf-18). |
| Prefill row fingerprints (`FQ_ROW_HASH=<path>`) | unset, path | unset | Diagnostic, not a setting. Hashes each row on the GPU in sixteen stages: the residual plane at three points of every layer (its input, after the attention block, after the routed tail), six inside the attention block of full-attention layers (the queries, the QSA selection, the attention core output, the block output after the gate and o_proj, and the keys and values after the RoPE and norm epilogue), three inside the indexer (its query projection, the raw key timeline as this chunk writes it, and the **pooled keys** — rows there are blocks, not positions), two for the K/V **cache** rows the copy writes and the attention reads, and two for the split-KV accumulator (`m`, `d`) the row's merge folded — the one buffer in the attention path nothing else covered. They share one dimension, so one dump and one diff cover both layer kinds. One readback at the prefill/decode boundary, so nothing is added to the run’s shape; the buffer is 8 bytes x 16 stages x layers per position — 50 MB at an 8k prefill on the 125B. Diff two dumps with `tools/row-hash-diff.py a.bin b.bin`, which reports the first (layer, stage, row) that moved and what that stage implies. What it has established, in the order the stages were added: the first landings were a full-attention layer’s own output (layer 3 row 1024, later layer 7 at rows 2064 and 2304) with the layer’s input, its q/k/v projections and, at layer 3, its chosen cells all bit-identical; then, with the indexer’s stages added, the ranking’s own inputs — its query, its raw key timeline — were identical while the cells differed; and with the **pooled keys** hashed last, the first landing is now **identical pooled keys with differing cells**, which puts the locus in score → radix-select → cells-write and excludes the pool’s multi-chunk page assembly. The two K/V-cache stages were added last, because stages 7/8 fingerprint the epilogue's scratch and the attention reads the cache: with them, an attention output that differs on rows the map reports as equal separates the copy from the kernel. What they show so far: the second mechanism first differs inside layer 3's attention block at row 1024 with the query, key and value *stage* rows all identical, and the cache stages report clean beside them. That locus was then traced one step further back — to the indexer *store* that fed the ranking, which was writing every prefill key at twice its position — and fixed; the pairs that diverged now reproduce bit-for-bit. See [KV-15](experiments/summaries/05-attention-and-kv-cache.md#kv-15). |
| GDN sub-stage split (`FQ_GDN_SPLIT=1`) | unset, 1 | unset | Diagnostic, not a setting. Commits the GDN layer’s four sub-stages as separate command buffers so each reports its own GPU time through the existing `gpu_gdn_*_ms/split=` fields on the prefill-scoped counters line. Nothing waits where a buffer is committed — only the submission order matters, so the layer’s existing wait completes them all and the sync pattern, and so the overlap, is unchanged. **It does change `gpu_cb1_gdn`:** the GDN work moves into the stage buffers, so cb1 falls to the layer’s post-GDN remainder (~260 ms against ~13,150 ms unsplit). Read the stages against the unsplit total; they sum to 95% of it. Its own overhead is measured, not assumed: prefill wall is 29.95/29.53 s with it off against 29.51/29.42 s with it on. On the 125B it puts 8.66 s of a 12.64 s GDN stack in the input projections, 3.24 s in the output projection, 0.72 s in the chunked recurrent scan and 0.009 s in the conv — see [PF-18](experiments/summaries/06-prefill.md#pf-18). |
...
**Test-scoped knobs.** Three environment variables belong to the test suite
rather than to a run. `FQ_ATTN_FUZZ_ROUNDS` (default 200) sets how many times
the prefill attention determinism tests re-dispatch the dense and the cells
path at each of nine lengths straddling 1,024 and 2,051 — a pass bounds the
kernels' nondeterminism rate at the number of dispatches run; it does not clear
them. `FQ_CHAINED` and `FQ_HEAD` switch the Qwen layer-replay diagnostics from
per-layer isolation to a chained replay and add the final head replay at the
last layer. The install-gated heavy diagnostics are selected by name in
`tools/heavy-tests.sh`, which is the suite's authority for serial runs.
