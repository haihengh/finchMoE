...
| Prefill row fingerprints (`FQ_ROW_HASH=<path>`) | unset, path | unset | Diagnostic, not a setting. Hashes each row on the GPU: the residual plane at three points of every layer (its input, after the attention block, after the routed tail) and, on full-attention layers, four more inside the attention block — the q/k/v output after the RoPE and norm epilogue, the QSA selection, the attention output, and the block output after the gate and o_proj. The four extra stages are what separates "the projections moved" from "the selection moved", which the plane alone cannot tell apart. One readback at the prefill/decode boundary, so nothing is added to the run’s shape; the buffer is 8 bytes x 7 stages x layers per position (21 MiB at 8k context). Diff two dumps with `tools/row-hash-diff.py a.bin b.bin`, which reports the first (layer, stage, row) that moved and what that stage implies. What it found: the long-prompt nondeterminism is born in **one row of one layer’s attention output** — layer 3, row 1024, with the projections and the selection bit-identical there ([KV-15](experiments/summaries/05-attention-and-kv-cache.md#kv-15)). |
