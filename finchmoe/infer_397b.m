/*
 * infer.m — Complete Qwen3.5-397B-A17B inference engine using Metal
 *
...
#define GATE_S_OFF_4  2097152
#define GATE_B_OFF_4  2228224
#define UP_W_OFF_4    2359296
#define UP_S_OFF_4    4456448
#define UP_B_OFF_4    4587520
#define DOWN_W_OFF_4  4718592
#define DOWN_S_OFF_4  6815744
#define DOWN_B_OFF_4  6946816
...
#define EXPERT_BITS (g_use_1bit ? 1 : (g_use_2bit ? 2 : (g_use_int8 ? 8 : 4)))
...
#define MODEL_PATH_DEFAULT "../models/Qwen3.5-397B-A17B-4bit-custom"
...
static inline size_t active_expert_size(void) {
    return g_use_1bit ? EXPERT_SIZE_1BIT : (g_use_2bit ? EXPERT_SIZE_2BIT : (g_use_int8 ? EXPERT_SIZE_8BIT : EXPERT_SIZE_4BIT));
}
...
static void gpu_encode_batch_matvec(
    ...
    return;  // Done — all specs encoded (BF16 on GPU gemv_bf16, 4bit on matvec_v3/fast)
    for (int i = 0; i < num_specs; i++) {
        BatchMatvecSpec *s = &specs[i];
        NSUInteger w_off = (NSUInteger)((const char *)s->W      - (const char *)[ctx->wf_buf contents]);
        NSUInteger s_off = (NSUInteger)((const char *)s->scales  - (const char *)[ctx->wf_buf contents]);
