...
enum ResidentIndexReader {
    static let defaultMaxBytes = FQTurboFormatV1.residentIndexMaxBytes
    ...
    package static func load(fileDescriptor fd: Int32,
                             displayPath: String,
                             maxBytes: UInt64 = defaultMaxBytes) throws -> ResidentIndex {
        let headerBytes = FQTurboFormatV1.residentHeaderBytes
        ...
        let wireHeader: FQTurboResidentIndexHeaderV1
        do {
            wireHeader = try headerBuf.withUnsafeBytes {
                try FQTurboResidentIndexCodec.decodeHeader($0)
            }
        } catch {
        ...
        let wireEntries: [FQTurboResidentIndexEntryV1]
        do {
            wireEntries = try indexBuf.withUnsafeBytes {
                try FQTurboResidentIndexCodec.decodeRegion($0, header: wireHeader)
            }
        } catch {
            throw ModelError.indexCorrupt(detail: "\(error)")
        }
