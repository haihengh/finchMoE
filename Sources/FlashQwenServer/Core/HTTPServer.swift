...
public actor FlashQwenHTTPServer {
...
private final class ServerHTTPHandler: ChannelInboundHandler, @unchecked Sendable {
    ...
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        case .head(let head):
            ...
            oversized = false
        case .body(var part):
            if body.readableBytes + part.readableBytes > FlashQwenHTTPServer.maximumBodyBytes {
                oversized = true
            } else {
    ...
    private func handleAsyncError(_ error: Error,
        ...
        if let requestError = error as? ServerRequestError {
            ...
            envelope = OpenAIErrorEnvelope(
                message: "generation failed; see FlashQwenServer stderr",
                code: "internal_error",
                type: "server_error")
        }
