...
import FlashQwenMacPresentation
import SwiftUI
    ...
    private var identity: some View {
        VStack(spacing: 10) {
            Image(systemName: "externaldrive.badge.plus")
                ...
                .foregroundStyle(FlashQwenMacTheme.accentColor)
                .accessibilityHidden(true)
            Text("Model required")
                .font(.title.bold())
                .accessibilityHeading(.h1)
            Text("FlashQwen needs \(model.installDescriptor.displayName) before it can generate text.")
                .font(.body)
