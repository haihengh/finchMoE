...
/// Writes the Qwen `.finchturbo` payloads: the resident `model_weights.bin`
...
/// the canonical `FinchTurboQuantization`, `(1 + w)` norm baking, and bf16 →
...
enum QwenQuantizedWriter {
    ...
    private static func writeAffine(rows: Int, cols: Int, bits: Int,
                                    ...
                                    biasOffset: UInt64,
                                    audit: RepackAudit) throws {
        let groups = cols / FinchTurboQuantization.groupSize
        ...
        while row < rows {
            ...
            for i in 0..<batch {
                ...
                for k in 0..<cols {
                    ...
                    floats[k] = FinchTurboQuantization.bf16ToFloat(bits16)
                }
                if bits == 4 {
                    let q = floats.withUnsafeBufferPointer {
                        FinchTurboQuantization.quantizeInt4Affine($0, count: cols)
                    }
                    ...
                    let q = floats.withUnsafeBufferPointer {
                        FinchTurboQuantization.quantizeInt8Affine($0, count: cols)
                    }
                    q.packed.withUnsafeBytes { raw in
                        packedBatch.withUnsafeMutableBytes { dst in
                            memcpy(dst.baseAddress!.advanced(by: i * packedRowBytes),
    ...
    struct QwenRowScratch {
        ...
        mutating func decodeBf16Row(_ bytes: UnsafeRawBufferPointer,
            ...
            for i in 0..<count {
                ...
                floats[i] = FinchTurboQuantization.bf16ToFloat(bits)
            }
        ...
        mutating func encodeBf16OnePlusW(_ values: [Float]) -> [UInt16] {
            for (i, v) in values.enumerated() {
                words[i] = FinchTurboQuantization.bf16Bits(1.0 + v)
            }
