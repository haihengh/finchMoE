...
package struct FQTurboResidentIndexHeaderV1: Equatable, Sendable {
...
package struct FQTurboResidentIndexEntryV1: Equatable, Sendable {
...
package enum FQTurboResidentIndexCodec {
    package static func decodeHeader(_ bytes: UnsafeRawBufferPointer) throws -> FQTurboResidentIndexHeaderV1 {
        guard bytes.count >= FQTurboFormatV1.residentHeaderBytes else {
            throw FQTurboFormatError.truncated(field: "resident.header")
        }
        let base = bytes.baseAddress!
        return FQTurboResidentIndexHeaderV1(
    ...
    package static func decodeRegion(_ bytes: UnsafeRawBufferPointer,
                                     header: FQTurboResidentIndexHeaderV1) throws -> [FQTurboResidentIndexEntryV1] {
        guard header.indexSize <= FQTurboFormatV1.residentIndexMaxBytes else {
        ...
        guard header.indexSize <= UInt64(bytes.count),
              header.indexSize >= UInt64(FQTurboFormatV1.residentHeaderBytes),
              header.indexSize % FQTurboFormatV1.alignmentBytes == 0 else {
            throw FQTurboFormatError.truncated(field: "resident.index")
        }
        let tableBytes = try fqturboCheckedMultiply(header.entryCount,
                                                   UInt64(FQTurboFormatV1.residentEntryBytes),
                                                   field: "resident.entryTable")
        let tableEnd = try fqturboCheckedAdd(UInt64(FQTurboFormatV1.residentHeaderBytes),
                                            tableBytes, field: "resident.entryTable")
        guard tableEnd <= header.indexSize, header.entryCount <= UInt64(Int.max) else {
            throw FQTurboFormatError.invalid(field: "resident.entryTable", reason: "outside index")
        }
        let residentEnd = try fqturboCheckedAdd(header.indexSize, header.residentSize,
                                               field: "resident.payload")
        let base = bytes.baseAddress!
        var result: [FQTurboResidentIndexEntryV1] = []
        ...
        for index in 0..<Int(header.entryCount) {
            let offset = FQTurboFormatV1.residentHeaderBytes + index * FQTurboFormatV1.residentEntryBytes
            ...
            guard readU8(entry, 7) == 0 else {
                throw FQTurboFormatError.invalid(field: "resident.entries[\(index)].reserved",
                                                reason: "must be zero")
            }
            let nameEnd = try fqturboCheckedAdd(nameOffset, nameLength,
                                               field: "resident.entries[\(index)].name")
            guard nameOffset >= tableEnd, nameEnd <= header.indexSize,
                  nameLength <= UInt64(Int.max) else {
                throw FQTurboFormatError.invalid(field: "resident.entries[\(index)].name",
            ...
            guard FQTurboFormatV1.DType(rawValue: dtype) != nil else {
                throw FQTurboFormatError.invalid(field: "resident.entries[\(index)].dtype",
            ...
            result.append(FQTurboResidentIndexEntryV1(
    ...
    private static func validatePrimaryPayloadRange(offset: UInt64, size: UInt64,
                                                    ...
                                                    field: String) throws {
        guard size > 0 else {
            throw FQTurboFormatError.invalid(field: field, reason: "primary payload is empty")
        }
    ...
    private static func validateOptionalPayloadRange(offset: UInt64, size: UInt64,
        ...
        if size == 0 {
            guard offset == 0 else {
                throw FQTurboFormatError.invalid(field: field, reason: "absent payload offset must be zero")
            }
            return
        }
        try validateContainedPayloadRange(offset: offset, size: size,
    ...
    private static func validateContainedPayloadRange(offset: UInt64, size: UInt64,
                                                      ...
                                                      residentEnd: UInt64,
                                                      field: String) throws {
        let end = try fqturboCheckedAdd(offset, size, field: field)
        guard offset >= header.indexSize, end <= residentEnd else {
            throw FQTurboFormatError.invalid(field: field, reason: "range outside resident payload")
        }
    }
