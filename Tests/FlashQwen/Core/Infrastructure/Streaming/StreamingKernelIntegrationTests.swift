...
/// End-to-end: write a synthetic .fqturbo blob containing one "expert" worth of
...
@Suite struct StreamingKernelIntegrationTests {
    ...
    /// Build the in-memory bytes that we'll write to the fake .fqturbo.
    ...
    @Test func endToEnd_streamFromFile_runKernel_matchesCpuReference() throws {
        ...
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("flash-qwen-smoke-\(UUID().uuidString).bin")
