...
@Suite struct VerifiedInstallTests {
    @Test func acceptsRootSymlinkAndBindsAlias() throws {
        ...
        let alias = temporaryURL("verified-install-link.finchturbo")
        defer {
        ...
        let result = try VerifiedInstallTool.run(
            options: VerifyInstallOptions(inputFinchTurbo: alias.path))
    ...
    @Test func concurrentWritersPublishCompleteReceipt() async throws {
            ...
            for _ in 0..<2 {
                group.addTask {
                    _ = try VerifiedInstallTool.run(
                        options: VerifyInstallOptions(inputFinchTurbo: root.path))
                }
            }
        ...
        #expect(!names.contains { $0.hasPrefix(".finchturbo-") && $0.hasSuffix(".tmp") })
    }
    ...
    private func makeInstall(manifestBytes: UInt64? = nil,
        ...
        let root = temporaryURL("verified-install.finchturbo")
        try FileManager.default.createDirectory(
            at: root.appendingPathComponent("packed_experts"),
            withIntermediateDirectories: true)
        let stride = FinchTurboFormatV1.alignmentBytes
        let modelData = Data([0])
        let layerData = Data(repeating: 0, count: Int(stride))
        var layoutData = try FinchTurboPackedExpertsLayoutCodec.encode(
            FinchTurboPackedExpertsLayoutV1(
                ...
                layers: [FinchTurboLayerV1(
                    layer: 0,
                    file: "layer_00.bin",
                    experts: [FinchTurboExpertV1(
                        expert: 0,
                        physicalRank: nil,
                        offset: 0,
                        size: stride,
                        tensors: ["gate": FinchTurboSubTensorV1(
                            offset: 0,
                            size: 32,
                            dtype: "U32",
                            shape: [8, 8],
        ...
        let files = [
            "model_weights.bin": FinchTurboManifestFileV1(
                size: UInt64(modelData.count), sha256: hash(modelData)),
            "packed_experts/layout.json": FinchTurboManifestFileV1(
        ...
        var manifestData = try FinchTurboManifestCodec.encode(FinchTurboManifestV1(
            flags: [
    ...
    private func fixtureArch() -> FinchTurboManifestArchV1 {
        FinchTurboManifestArchV1(
            hiddenSize: 64,
            ffnIntermediate: 128,
    ...
    private func temporaryURL(_ suffix: String) -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("finchturbo-\(UUID().uuidString)-\(suffix)")
    }
}
