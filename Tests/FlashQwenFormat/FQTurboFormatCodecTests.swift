import Foundation
import Testing
@testable import FlashQwenFormat
...
private enum FormatFixture {
    ...
    static func manifest(sourceSnapshotHash: String? = "snapshot",
                         quant: FQTurboManifestQuantV1? = FormatFixture.quant,
                         bitWidthOverridesHonored: Int? = 120,
                         minor: Int = 0) -> FQTurboManifestV1 {
        FQTurboManifestV1(
            versionMinor: minor,
            flags: [
            ...
            quant: quant,
            files: [
                "model_weights.bin": FQTurboManifestFileV1(size: 16_448, sha256: zeroSHA),
                "packed_experts/layout.json": FQTurboManifestFileV1(size: 1, sha256: zeroSHA),
                "packed_experts/layer_00.bin": FQTurboManifestFileV1(
                    size: 2 * FQTurboFormatV1.alignmentBytes, sha256: zeroSHA),
            ],
            expertsPerLayer: 2,
            numLayers: 1,
    ...
    static let quantSlot = FQTurboManifestQuantSlotV1(
    ...
    static func layout(explicitIDs: Bool = true,
        ...
        let tensor = FQTurboSubTensorV1(
            offset: 0, size: 32, dtype: "U32", shape: [8, 8], bits: 4)
        return FQTurboPackedExpertsLayoutV1(
            expertStride: FQTurboFormatV1.alignmentBytes,
            numLayers: 1,
            expertsPerLayer: 2,
            layers: [FQTurboLayerV1(
                ...
                experts: (0..<2).map { expert in
                    FQTurboExpertV1(
                        ...
                        offset: UInt64(expert) * FQTurboFormatV1.alignmentBytes,
                        size: FQTurboFormatV1.alignmentBytes,
                        tensors: ["gate": tensor])
                })])
    }
    ...
    static func residentBytes(nameOffset: UInt32 = 96,
                              ...
                              dtype: UInt8 = FQTurboFormatV1.DType.u32.rawValue,
                              reserved: UInt8 = 0,
                              shape: [UInt32] = [1, 1, 0, 0],
                              fileOffset: UInt64 = FQTurboFormatV1.alignmentBytes,
                              sizeBytes: UInt64 = 16,
                              scaleOffset: UInt64 = FQTurboFormatV1.alignmentBytes + 16,
                              scaleSize: UInt64 = 8,
                              biasOffset: UInt64 = FQTurboFormatV1.alignmentBytes + 24,
                              biasSize: UInt64 = 8,
                              header: FQTurboResidentIndexHeaderV1 = .init(
                                indexSize: FQTurboFormatV1.alignmentBytes,
                                residentSize: 64, entryCount: 1)) -> Data {
        var bytes = Data(repeating: 0, count: Int(header.indexSize))
        bytes.withUnsafeMutableBytes { raw in
            FQTurboResidentIndexCodec.writeHeader(into: raw.baseAddress!, header: header)
            guard raw.count >= 96 else { return }
            let entry = FQTurboResidentIndexEntryV1(
            ...
            FQTurboResidentIndexCodec.writeEntry(
                into: raw.baseAddress!.advanced(by: FQTurboFormatV1.residentHeaderBytes),
                entry: entry, nameOffset: nameOffset)
            raw[FQTurboFormatV1.residentHeaderBytes + 7] = reserved
...
@Suite struct FQTurboManifestCodecTests {
    @Test func roundTripPreservesOptionalPresenceAndWriterField() throws {
        let manifest = FormatFixture.manifest()
        let encoded = try FQTurboManifestCodec.encode(manifest)
        #expect(try FQTurboManifestCodec.decode(encoded) == manifest)
    ...
    @Test func acceptsAdditiveMinorAndUnknownTopLevelKey() throws {
        let data = try FQTurboManifestCodec.encode(FormatFixture.manifest(minor: 9))
        ...
        #expect(try FQTurboManifestCodec.decode(changed).versionMinor == 9)
    }
    ...
    func acceptsAbsentLegacyOptionalField(_ key: String) throws {
        let data = try FQTurboManifestCodec.encode(FormatFixture.manifest())
        var root = try #require(JSONSerialization.jsonObject(with: data) as? [String: Any])
        root.removeValue(forKey: key)
        let changed = try JSONSerialization.data(withJSONObject: root)
        let decoded = try FQTurboManifestCodec.decode(changed)
    ...
    @Test func rejectsUnsafeManifestPaths() throws {
        var root = try #require(JSONSerialization.jsonObject(
            with: FQTurboManifestCodec.encode(FormatFixture.manifest())) as? [String: Any])
        var files = try #require(root["files"] as? [String: Any])
        files["../escape.bin"] = files.removeValue(forKey: "model_weights.bin")
        root["files"] = files
        let data = try JSONSerialization.data(withJSONObject: root)
        #expect(throws: FQTurboFormatError.self) { try FQTurboManifestCodec.decode(data) }
    }
...
@Suite struct FQTurboPackedExpertsLayoutCodecTests {
    @Test func roundTripPreservesIdentityFallback() throws {
        let layout = FormatFixture.layout(explicitIDs: false, explicitRanks: false)
        let encoded = try FQTurboPackedExpertsLayoutCodec.encode(layout)
        #expect(try FQTurboPackedExpertsLayoutCodec.decode(encoded) == layout)
    }
    ...
    @Test func rejectsMixedExplicitAndPositionalIDs() throws {
        ...
        let experts = [
            ...
            FQTurboExpertV1(
                ...
                offset: FQTurboFormatV1.alignmentBytes,
                size: FQTurboFormatV1.alignmentBytes,
        ...
        let layout = FQTurboPackedExpertsLayoutV1(
            ...
            layers: [FQTurboLayerV1(layer: 0, file: "layer_00.bin", experts: experts)])
        #expect(throws: FQTurboFormatError.self) {
            try FQTurboPackedExpertsLayoutCodec.encode(layout)
        }
    }
    ...
    @Test func crossValidationRequiresExactLayerManifestEntry() throws {
        ...
        sizes["packed_experts/./layer_00.bin"] = 2 * FQTurboFormatV1.alignmentBytes
        #expect(throws: FQTurboFormatError.self) {
            try FQTurboV1StructuralValidator.crossValidate(
                manifestNumLayers: 1, manifestExpertsPerLayer: 2,
                manifestExpertStride: FQTurboFormatV1.alignmentBytes,
                manifestFileSizes: sizes, layout: FormatFixture.layout())
        }
    }
}

@Suite struct FQTurboResidentIndexCodecTests {
    @Test func decodesValidRegion() throws {
        let header = FQTurboResidentIndexHeaderV1(
            indexSize: FQTurboFormatV1.alignmentBytes, residentSize: 64, entryCount: 1)
        let bytes = FormatFixture.residentBytes(header: header)
        let entries = try bytes.withUnsafeBytes {
            try FQTurboResidentIndexCodec.decodeRegion($0, header: header)
        }
    ...
    @Test func rejectsOverlappingResidentPayloadRanges() throws {
        ...
        let bytes = FormatFixture.residentBytes(
            scaleOffset: FQTurboFormatV1.alignmentBytes + 8,
            header: header)
        #expect(throws: FQTurboFormatError.self) {
    ...
    @Test func rejectsNameInsideEntryTable() throws {
        let header = FQTurboResidentIndexHeaderV1(
            indexSize: FQTurboFormatV1.alignmentBytes, residentSize: 64, entryCount: 1)
        let bytes = FormatFixture.residentBytes(
            nameOffset: UInt32(FQTurboFormatV1.residentHeaderBytes), header: header)
        #expect(throws: FQTurboFormatError.self) {
            try bytes.withUnsafeBytes { try FQTurboResidentIndexCodec.decodeRegion($0, header: header) }
        }
    }
    ...
    @Test func rejectsPayloadRangeOutsideResidentRegion() throws {
        let header = FQTurboResidentIndexHeaderV1(
            indexSize: FQTurboFormatV1.alignmentBytes, residentSize: 64, entryCount: 1)
        let bytes = FormatFixture.residentBytes(
            fileOffset: FQTurboFormatV1.alignmentBytes + 56,
    ...
    @Test func rejectsUnalignedIndexRegion() throws {
        let header = FQTurboResidentIndexHeaderV1(indexSize: 128, residentSize: 64, entryCount: 1)
        let bytes = FormatFixture.residentBytes(
    ...
    @Test func rejectsAbsentCompanionWithNonzeroOffset() throws {
        let header = FQTurboResidentIndexHeaderV1(
            indexSize: FQTurboFormatV1.alignmentBytes, residentSize: 64, entryCount: 1)
        let bytes = FormatFixture.residentBytes(
            scaleOffset: FQTurboFormatV1.alignmentBytes + 16, scaleSize: 0,
...
@Suite struct FQTurboV1StructuralValidatorTests {
    ...
    @Test func rejectsTensorOutsideExpertBlob() throws {
        ...
        let layout = FQTurboPackedExpertsLayoutV1(
            ...
            layers: [FQTurboLayerV1(layer: 0, file: "layer_00.bin", experts: [expert])])
        #expect(throws: FQTurboFormatError.self) {
            try FQTurboV1StructuralValidator.validate(layout)
        }
    }
