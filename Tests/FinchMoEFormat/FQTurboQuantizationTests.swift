...
@Suite struct FQTurboQuantizationTests {

    /// Alternating ±2^-123 BF16 values, exactly as found in the checkpoint.
    private static func subnormalResidueRow() -> [Float] {
        (0..<FQTurboQuantization.groupSize).map { k in
            FQTurboQuantization.bf16ToFloat(k % 2 == 0 ? 0x0200 : 0x8200)
        }
    }
    ...
    @Test func int8SubnormalResidueRowQuantizesWithoutTrap() {
        ...
        let q = row.withUnsafeBufferPointer {
            FQTurboQuantization.quantizeInt8Affine($0, count: row.count)
        }
        let decoded = FQTurboQuantization.dequantizeInt8Affine(q, n: row.count)
    ...
    @Test func int4SubnormalResidueRowQuantizesWithoutTrap() {
        let row = Self.subnormalResidueRow()
        let q = row.withUnsafeBufferPointer {
            FQTurboQuantization.quantizeInt4Affine($0, count: row.count)
        }
        let decoded = FQTurboQuantization.dequantizeInt4Affine(q, n: row.count)
        for (a, b) in zip(decoded, row) {
            #expect(abs(a - b) < 1e-36)
        }
    }
    ...
    @Test func ordinaryRangeStillRoundTripsWithinAffineError() {
        ...
        let row = (0..<FQTurboQuantization.groupSize).map {
