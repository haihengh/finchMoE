...
@Suite struct AppModelLoadStateTests {
    ...
    @Test func runRequiresReadyState() {
        ...
        #expect(!model.canRun)
        model.loadState = .ready(modelDirectory: URL(fileURLWithPath: "/tmp/m.finchturbo"), loadSeconds: 1.2)
        #expect(model.canRun)
    }
    ...
    @Test func staleReadyStateForOldModelPathIsIgnored() {
        ...
        let oldURL = URL(fileURLWithPath: "/tmp/old.finchturbo")
        let newURL = URL(fileURLWithPath: "/tmp/new.finchturbo")
