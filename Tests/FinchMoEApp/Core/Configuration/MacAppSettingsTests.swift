...
@Suite struct MacAppSettingsTests {
    @Test func settingsFileLivesBesideModelDirectory() {
        let model = URL(fileURLWithPath: "/tmp/FinchMoE/gemma4.finchturbo",
                        isDirectory: true)
    ...
    @Test func missingFileCreatesReadableDefaults() throws {
        ...
        let model = root.appendingPathComponent("gemma4.finchturbo", isDirectory: true)
    ...
    @Test func appModelLoadsAndSavesPersistedSettings() throws {
        let root = try makeTemporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        let modelDirectory = root.appendingPathComponent("gemma4.finchturbo", isDirectory: true)
    ...
    @Test func changingModelDirectoryLoadsItsNewlineShortcut() throws {
        let root = try makeTemporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        let first = root.appendingPathComponent("first/model.finchturbo", isDirectory: true)
        let second = root.appendingPathComponent("second/model.finchturbo", isDirectory: true)
