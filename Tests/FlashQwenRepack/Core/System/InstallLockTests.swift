...
@Suite struct InstallLockTests {
    @Test func twoContendersForCanonicalTargetCannotBothAcquire() throws {
        ...
        let output = (root as NSString).appendingPathComponent("model.fqturbo")
    ...
    @Test func symlinkedParentAliasesContendOnOnePhysicalLock() throws {
        ...
        let first = try InstallLock.acquire(
            outputDirectory: (physical as NSString).appendingPathComponent("model.fqturbo"))

        #expect(throws: RepackError.self) {
            _ = try InstallLock.acquire(
                outputDirectory: (alias as NSString).appendingPathComponent("model.fqturbo"))
        }
    ...
    private func temporaryRoot(_ tag: String) -> String {
        let path = (NSTemporaryDirectory() as NSString)
            .appendingPathComponent("flashqwen-lock-\(tag)-\(UUID().uuidString)")
