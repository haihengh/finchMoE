...
@Suite struct ManifestReaderTests {
    ...
    static func writeToyManifest(_ overrides: [String: Any] = [:],
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("fqturbo-manifest-test-\(UUID().uuidString)")
    ...
    @Test func missingManifestThrowsPartialInstall() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("fqturbo-empty-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
        #expect {
    ...
    @Test func wrongMagicThrowsNotAFQTurboDirectory() throws {
        let (dir, toy) = try Self.writeToyManifest(["magic": "NOT_FQTURBO"])
        defer { try? FileManager.default.removeItem(at: dir) }
        #expect(throws: ModelError.notAFQTurboDirectory) {
    ...
    @Test func removedQuantKVFlagIsRejected() throws {
        let (dir, toy) = try Self.writeToyManifest(flags: ["streamingPresent": true,
                                                           "quantKV": true,
                                                           "aneSharedExpert": false])
        defer { try? FileManager.default.removeItem(at: dir) }
        #expect {
            ...
            return detail.contains("removed quantized-KV")
        }
    }
