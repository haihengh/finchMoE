...
@Suite struct Sha256VerifierTests {
    ...
    @Test func hashesEmptyFile() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("finchturbo-sha-empty-\(UUID().uuidString).bin")
    ...
    @Test func chunkSizeDoesNotAffectDigest() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("finchturbo-sha-2m-\(UUID().uuidString).bin")
    ...
    @Test func verifyMatches() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("finchturbo-sha-match-\(UUID().uuidString).bin")
    ...
    @Test func verifyMismatchThrowsChecksumMismatch() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("finchturbo-sha-bad-\(UUID().uuidString).bin")
